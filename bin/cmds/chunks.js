...
exports.handler = async function (argv) {
  ...
  for (const coords in level.chunks) {
    ...
    if (chunk.Data2D) {
      ...
      console.log(`Chunk at X=${chunk.X} Z=${chunk.Z} has data2d ${d.length}`)
      const elevations = []
      const biomes = []
      for (let i = 0; i < 256; i++) {
      ...
      console.log('elevations', elevations.slice(16))
      console.log('biomes', biomes.slice(16))
    }
  }
  console.log(`${count} of ${Object.keys(level.chunks).length} chunks had data2d`)
}
