...
// indexed by block name, "textures" attribute references index in textureTable
...
module.exports = class Cache {
  ...
  async loadTexture (block, x, y, z, blockY) {
    ...
    if (!this.get(block, blockY)) {
      ...
      if (block.states) {
        if (block.states.growth) {
          file = path.resolve('resourcepacktemplate',
            textureTable.texture_data[texture].textures[block.states.growth])
        } else if (block.states[`${name}_type`]) {
      ...
      log.debug('new texture %o file %s', block, file)
