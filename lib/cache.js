...
module.exports = class Cache {
  ...
  async loadTexture (block, x, y, z, blockY) {
    ...
    if (!this.get(block, blockY)) {
      ...
      let texture = blockTable[block.name.slice(10)].textures
      let file
      // First see if we have a state, that shortcuts us directly to a texture
      // minecraft:stone has stone_type (i.e. granite_smooth)
      // minecraft:dirt has dirt_type
      // minecraft:ladder has facing_direction
      if (block.states && block.states[`${name}_type`]) {
        const type = block.states[`${name}_type`] // i.e. granite_smooth
        // normal grass or stone type stone
        if (type !== 'normal') {
      ...
      if (!file) {
        ...
        if (patchTable[texture] && patchTable[texture].textures[block.name]) {
          ...
          if (!textureTable.texture_data[texture].textures) {
            log.error(`Texture not in table: ${texture} (${block.name})`)
      ...
      if (await fs.exists(path.normalize(`${file}.png`))) {
        ...
        // TGA Loading
        imageBuffer = await tga2png(await fs.readFile(file + '.tga'))
          .catch((err) => {
            log.error(`Failed to load TGA for ${name} ${block.name} ${texture}`)
