    ...
    if ([1, 8, 9].includes(subChunkVersion)) {
      ...
      for (let storage = 0; storage < storages; storage++) {
        ...
        if (!isRuntime) {
          ...
          // The nbt data is after the words, but we need it before we iterate
          // through the words
          ...
          // TODO this appears to be buggy when bits per block requires bit shifting
          // console.log(`blocksPerWord ${blocksPerWord}`)
          for (let wordi = 0; wordi < wordCount; wordi++) {
            // console.log(`position ${position}`)
            const word = data.readInt32LE(wordOffset)
            wordOffset += 4

            for (let block = 0; block < blocksPerWord && position < 4096; block++) {
